    ...
    <footer className="bg-stone-950 text-stone-100 py-24 sm:py-32 px-6 sm:px-12 border-t border-stone-800">
      <div className="max-w-7xl mx-auto space-y-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-6 grid grid-cols-2 sm:grid-cols-3 gap-8 text-xs font-mono">
            <div className="space-y-3">
              <span className="block text-[9px] text-stone-500 uppercase tracking-widest">PORTFOLIO</span>
              <ul className="space-y-2 text-stone-300">
                <li><button onClick={() => onNavigate("projects")} className="hover:text-white cursor-pointer font-bold">Selected Works →</button></li>
                <li><button onClick={() => onNavigate("studio")} className="hover:text-white cursor-pointer">Studio Philosophy</button></li>
                <li><button onClick={() => onNavigate("materiality")} className="hover:text-white cursor-pointer">Material Archive</button></li>
                <li><button onClick={() => onNavigate("services")} className="hover:text-white cursor-pointer">Services</button></li>
                <li><button onClick={() => onNavigate("journal")} className="hover:text-white cursor-pointer">Journal Essays</button></li>
                <li><button onClick={() => onNavigate("contact")} className="hover:text-white cursor-pointer">Commissions</button></li>
              </ul>
            </div>

            <div className="space-y-3">
              <span className="block text-[9px] text-stone-500 uppercase tracking-widest">INQUIRIES</span>
              <ul className="space-y-2 text-stone-300">
        ...
        <div className="pt-12 border-t border-stone-900 flex flex-col sm:flex-row items-center justify-between text-[10px] font-mono text-stone-500 space-y-4 sm:space-y-0">
          ...
          <div>ARCHITECTURE & INTERIORS PORTFOLIO</div>
        </div>
      </div>
    </footer>
  );
